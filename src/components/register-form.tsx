...
export function RegisterForm() {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try { const form = new FormData(event.currentTarget); const payload = { fullName: String(form.get("fullName") ?? ""), nationalId: String(form.get("nationalId") ?? ""), email: String(form.get("email") ?? ""), phone: String(form.get("phone") ?? ""), password: String(form.get("password") ?? ""), role: String(form.get("role") ?? "") };
  ...
  return <form onSubmit={onSubmit} className="space-y-4" noValidate>
    ...
    <Field label="Número de cédula" name="nationalId" inputMode="numeric" required errors={fieldErrors.nationalId} hint="10 dígitos" />
    <Field label="Correo electrónico" name="email" type="email" autoComplete="email" required errors={fieldErrors.email} />
    <Field label="Teléfono" name="phone" type="tel" autoComplete="tel" required errors={fieldErrors.phone} />
...
function Field({ label, name, type = "text", required, autoComplete, errors, hint, inputMode }: { label: string; name: string; type?: string; required?: boolean; autoComplete?: string; errors?: string[]; hint?: string; inputMode?: "numeric" }) {
  return <label className="block space-y-1.5"><span className="text-sm font-semibold text-navy">{label}{required ? <span className="text-orange"> *</span> : null}</span><input name={name} type={type} required={required} autoComplete={autoComplete} inputMode={inputMode} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-navy outline-none transition placeholder:text-slate-400 focus:border-sky focus:ring-4 focus:ring-sky/15" />{hint ? <span className="block text-xs text-slate-500">{hint}</span> : null}{errors?.[0] ? <span className="block text-sm text-red-700">{errors[0]}</span> : null}</label>;
}
