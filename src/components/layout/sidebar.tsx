...
export function Sidebar({ role, activePath }: SidebarProps) {
  ...
  return <aside className="hidden w-64 shrink-0 border-r border-border bg-surface lg:flex lg:flex-col"><Link href="/" className="flex items-center gap-3 px-6 py-6 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-sky/25"><span className="grid h-10 w-10 place-items-center rounded-md bg-navy text-lg font-bold text-white">M</span><span className="text-xl font-bold tracking-tight text-navy">Manta<span className="text-sky">360</span></span></Link><div className="min-h-0 flex-1 overflow-y-auto px-4"><p className="px-3 text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">{ROLE_LABELS[role]}</p><nav className="mt-3 space-y-1" aria-label="Navegación principal">{roleNavigation(role).map((item) => { const active = currentPath === item.href; const badge = role === "MUNICIPIO" ? badgeFor(item.href) : item.label === "Mensajes" ? unreadCount : 0; return <Link key={item.href} href={item.href} aria-current={active ? "page" : undefined} className={`flex items-center justify-between rounded-md px-3 py-2.5 text-sm font-semibold transition focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-sky/25 ${active ? "bg-sky/10 text-primary" : "text-muted hover:bg-surface-subtle hover:text-foreground"}`}><span>{item.label}</span>{badge > 0 ? <span className="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-semibold text-amber-800">{badge}</span> : null}</Link>; })}</nav></div><Link href="/" className="px-6 py-6 text-sm font-semibold text-muted transition hover:text-primary focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-sky/25">Volver al inicio</Link></aside>;
}
