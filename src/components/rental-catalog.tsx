...
export function RentalCatalog() {
  ...
  async function submitContact(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ propertyId: contactProperty.id, recipientId: contactProperty.landlord.id, content }) });
    const data = await response.json();
    if (!response.ok) { setError(data.error ?? "No se pudo enviar el mensaje"); return; }
    setContactSent(true);
  }
