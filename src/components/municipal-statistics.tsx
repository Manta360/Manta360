...
const statusColors = { PENDIENTE: "#d9972f", EN_PROCESO: "#1f6fa8", RESUELTO: "#2e9b69" };
...
export function municipalInsights(data: MunicipalStatisticsData): string[] {
  ...
  return [
    busiest ? `${busiest.zone} concentra la mayor cantidad de propiedades registradas.` : "Aún no hay propiedades clasificadas por zona.",
    open === 0 ? "No existen incidencias pendientes actualmente." : `${open} incidencia(s) siguen en atención.`,
    highestRent ? `La renta promedio más alta corresponde a ${highestRent.zone}.` : "Aún no hay rentas disponibles para comparar.",
  ];
}
...
function ChartTooltip({ active, payload, label, money = false }: { active?: boolean; payload?: Array<{ value?: number }>; label?: string; money?: boolean }) {
  ...
  return <div className="rounded-lg border border-border bg-surface px-3 py-2 text-sm shadow-lg"><p className="font-semibold text-navy">{label}</p><p className="mt-1 text-primary">{money ? currency.format(Number(payload[0]?.value ?? 0)) : `${payload[0]?.value ?? 0} propiedades`}</p></div>;
}
...
export function MunicipalStatistics({ data, loading, error }: Props) {
  ...
  return <section className="space-y-5">
    <div><p className="text-xs font-semibold uppercase tracking-[.18em] text-sky">Análisis global</p><h3 className="mt-2 text-2xl font-bold text-navy">Estadísticas municipales</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">Indicadores de propiedades aprobadas, renta, incidencias y arrendadores.</p></div>
