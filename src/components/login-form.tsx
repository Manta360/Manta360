...
export function LoginForm() {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch("/api/auth/login", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ identifier: String(form.get("identifier") ?? ""), password: String(form.get("password") ?? "") }) });
  ...
  return <form onSubmit={onSubmit} className="space-y-5" noValidate>
    <label className="block space-y-2"><span className="text-sm font-semibold text-navy">Correo electrónico o cédula</span><input name="identifier" required autoComplete="username" placeholder="tu@correo.com o 0000000000" className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-navy outline-none transition placeholder:text-slate-400 focus:border-sky focus:ring-4 focus:ring-sky/15" /></label>
