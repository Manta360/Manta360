...
export default async function HomePage() {
  ...
  return <div className="min-h-screen bg-background"><SiteHeader /><main>
    <section className="mx-auto grid w-full max-w-7xl items-center gap-10 px-5 py-14 sm:px-8 sm:py-16 lg:grid-cols-[1fr_.92fr] lg:gap-16 lg:py-20">
      <div className="max-w-2xl"><p className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[.18em] text-primary"><span className="h-2 w-2 rounded-full bg-sky" /> Vivienda y gestión en Manta</p><h1 className="mt-5 text-4xl font-bold leading-[1.08] tracking-tight text-navy sm:text-5xl lg:text-6xl">Encuentra tu próximo hogar en Manta.</h1><p className="mt-5 max-w-xl text-base leading-7 text-muted-foreground sm:text-lg">Explora propiedades verificadas, conecta con propietarios y gestiona tu alquiler de forma clara y segura.</p><div className="mt-7 flex flex-col gap-3 sm:flex-row"><Link href={primaryHref} className="landing-primary inline-flex min-h-11 items-center justify-center rounded-md px-5 py-3 text-sm font-semibold shadow-sm transition focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-sky/25">{panelHref ? "Ir a mi panel" : "Explorar propiedades"}</Link><Link href="#como-funciona" className="inline-flex min-h-11 items-center justify-center rounded-md border border-border bg-surface px-5 py-3 text-sm font-semibold text-primary transition hover:border-sky hover:bg-surface-subtle focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-sky/25">Conocer Manta360</Link></div><ul className="mt-7 flex flex-wrap gap-x-5 gap-y-2 text-sm font-medium leading-6 text-muted-foreground"><li className="flex items-center gap-2"><span className="grid h-4 w-4 place-items-center rounded-full bg-success/10 text-xs font-semibold text-success">✓</span> Propiedades verificadas</li><li className="flex items-center gap-2"><span className="grid h-4 w-4 place-items-center rounded-full bg-success/10 text-xs font-semibold text-success">✓</span> Gestión segura</li><li className="flex items-center gap-2"><span className="grid h-4 w-4 place-items-center rounded-full bg-success/10 text-xs font-semibold text-success">✓</span> Seguimiento municipal</li></ul></div>
      <div className="relative overflow-hidden rounded-2xl border border-border bg-surface p-2 shadow-md"><div className="min-h-[300px] rounded-xl bg-[url('https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&w=1600&q=85')] bg-cover bg-center sm:min-h-[390px]" role="img" aria-label="Costa de Manta"><div className="flex h-full min-h-[300px] items-end bg-gradient-to-t from-navy/55 via-transparent to-transparent p-4 sm:min-h-[390px] sm:p-6"><div className="max-w-xs rounded-lg border border-white/35 bg-white/94 p-3 text-navy shadow-sm sm:p-4"><p className="text-xs font-semibold uppercase tracking-[.15em] text-primary">Manta360</p><p className="mt-1 text-base font-semibold leading-6 sm:text-lg">Una experiencia de alquiler más clara para Manta.</p></div></div></div></div>
    </section>
