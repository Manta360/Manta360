...
import { finalizeContractAndSynchronizeProperty } from "@/lib/property-contract-state";
...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = await runContractTransaction(async (client) => {
      ...
      const outcome = await finalizeContractAndSynchronizeProperty(client, {
        contractId: id,
        propertyId: contract.propertyId,
        endedBy: session.sub,
        now,
      });
      if (!outcome.finalized) return { error: "El contrato cambio durante la finalizacion", status: 409 };
