...
const mocks = vi.hoisted(() => ({
  session: vi.fn(),
  listLandlords: vi.fn(), findLandlordById: vi.fn(), createLandlord: vi.fn(), updateLandlord: vi.fn(),
}));
vi.mock("@/lib/server-auth", () => ({ getActiveSession: mocks.session }));
vi.mock("@/repositories/admin-users.server", () => ({ adminUsersRepository: {
  listLandlords: mocks.listLandlords, findLandlordById: mocks.findLandlordById,
  createLandlord: mocks.createLandlord, updateLandlord: mocks.updateLandlord,
} }));
...
describe("KAN-39 - gestión administrativa de arrendadores PostgreSQL", () => {
  ...
  it("rechaza acceso no municipal", async () => {
    ...
    expect((await listLandlords()).status).toBe(403);
  });
