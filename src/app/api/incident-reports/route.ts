...
export async function POST(request: Request) {
  ...
  const contract = await incidentsRepository.findActiveContractForTenant(data.contractId, session.sub);
  ...
  const item = await incidentsRepository.create({ id: createTextId(), contractId: contract.id, propertyId: contract.propertyId, tenantId: contract.tenantId, landlordId: contract.landlordId, description: data.description, incidentDate, updatedAt: new Date() });
