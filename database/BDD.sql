-- =============================================================================
-- Manta360 — BDD.sql (entregable oficial de base de datos)
-- Ubicación: database/BDD.sql
-- Pontificia Universidad Católica del Ecuador — Sede Manabí
-- Carrera de Ingeniería de Software · Proyecto Fin de Semestre
--
-- Contiene el esquema COMPLETO (todas las tablas, enums, FKs e índices) y
-- un seed mínimo de demostración.
--
-- Uso (desde la raíz del repo):
--   psql "postgresql://USER:PASS@HOST:5432/postgres?sslmode=require" -f database/BDD.sql
--
-- Credenciales demo del seed (solo entorno de prueba):
--   municipio@manta360.demo     / Demo1234!
--   arrendador@manta360.demo    / Demo1234!
--   arrendatario@manta360.demo  / Demo1234!
--
-- Nota: database/schema.sql es el bootstrap canónico sin seed.
-- =============================================================================
...
-- =============================================================================
-- SEED MÍNIMO DE DEMOSTRACIÓN (idempotente)
...
INSERT INTO public.users (id, email, "passwordHash", "fullName", phone, "nationalId", role, active, "updatedAt")
VALUES
  (
    'demo-municipio',
    'municipio@manta360.demo',
    '$2b$12$6KEyWpg2SyuWwBUdZJpcqua1Qb/NPeye/ZBAOpTINpnmI9Z/RRREK',
    'Administrador Municipal Demo',
    '0990000001',
    '1000000001',
    'MUNICIPIO',
    true,
  ...
  (
    'demo-arrendador',
    'arrendador@manta360.demo',
    '$2b$12$6KEyWpg2SyuWwBUdZJpcqua1Qb/NPeye/ZBAOpTINpnmI9Z/RRREK',
    'Ana Arrendadora Demo',
    '0990000002',
    '1000000002',
    'ARRENDADOR',
    true,
  ...
  (
    'demo-arrendatario',
    'arrendatario@manta360.demo',
    '$2b$12$6KEyWpg2SyuWwBUdZJpcqua1Qb/NPeye/ZBAOpTINpnmI9Z/RRREK',
    'Luis Arrendatario Demo',
    '0990000003',
    '1000000003',
    'ARRENDATARIO',
    true,
...
INSERT INTO public.service_catalog (id, name, slug, active, "updatedAt")
VALUES
  ('11111111-1111-1111-1111-111111111111', 'Agua', 'agua', true, CURRENT_TIMESTAMP),
  ('22222222-2222-2222-2222-222222222222', 'Luz', 'luz', true, CURRENT_TIMESTAMP),
  ('33333333-3333-3333-3333-333333333333', 'Internet', 'internet', true, CURRENT_TIMESTAMP)
ON CONFLICT (name) DO UPDATE SET active = true, "updatedAt" = CURRENT_TIMESTAMP;

INSERT INTO public.properties (
  id, "landlordId", title, address, "monthlyRent", status, description, bedrooms, bathrooms,
  latitude, longitude, "createdBy", approved, "approvedAt", "approvedBy", "updatedAt"
) VALUES (
  'demo-property-centro',
  'demo-arrendador',
  'Departamento Centro Demo',
  'Centro de Manta, Calle 13 y Av. 24',
  450.00,
  'DISPONIBLE',
  'Propiedad de demostración aprobada para catálogo público y pruebas de flujo.',
  2,
  1,
  -0.9497470,
  -80.7330630,
...
ON CONFLICT (id) DO UPDATE SET
  ...
  status = 'DISPONIBLE',
  approved = true,
  "approvedBy" = 'demo-municipio',
  "updatedAt" = CURRENT_TIMESTAMP;

INSERT INTO public.property_services ("propertyId", "serviceId")
SELECT 'demo-property-centro', id FROM public.service_catalog WHERE name IN ('Agua', 'Luz', 'Internet')
ON CONFLICT DO NOTHING;

INSERT INTO public.contracts (
  id, "propertyId", "tenantId", "landlordId", "startDate", "endDate", status,
  "monthlyRent", city, purpose, "landlordSignedAt", "tenantSignedAt",
  "municipalReviewedAt", "municipalReviewedBy", "updatedAt"
) VALUES (
  'demo-contract-historial',
  'demo-property-centro',
  'demo-arrendatario',
  'demo-arrendador',
  '2025-01-01 00:00:00',
...
INSERT INTO public.incident_reports (
  id, "contractId", "propertyId", "tenantId", "landlordId", description, "incidentDate", status, "updatedAt"
) VALUES (
  'demo-incident-resuelto',
  'demo-contract-historial',
  'demo-property-centro',
  'demo-arrendatario',
  'demo-arrendador',
  'Fuga menor de agua en cocina — caso de demostración histórico.',
  '2025-03-15 00:00:00',
  'RESUELTO',
  CURRENT_TIMESTAMP
)
...
-- Tablas principales cubiertas: users (roles), properties, contracts, incident_reports (+ contract_requests en schema).
-- Fin de database/BDD.sql
